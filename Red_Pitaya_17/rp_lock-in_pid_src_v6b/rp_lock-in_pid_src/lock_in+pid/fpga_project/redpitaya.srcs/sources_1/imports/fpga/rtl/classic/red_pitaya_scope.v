...
module red_pitaya_scope #(
  // Ning: extended RAM from 2^14 to 2^16 (by a factor of 2^2 = 4)
  parameter RSZ = 16  // RAM size 2^RSZ
)(
   // ADC
   input                 adc_clk_i       ,  // ADC clock
   input                 adc_rstn_i      ,  // ADC reset - active low
   input      [  2-1: 0] osc_ctrl_i      ,  // oscilloscope control from lock module
...
wire [ 14-1: 0] adc_a_filt_out_m ;//Alexei: added
wire [ 14-1: 0] adc_a_filt_out ;
wire [ 14-1: 0] adc_b_filt_out_m ;//Alexei: added
...
reg               adc_trig_bn      ;
reg               adc_trig_sw      ;
//reg   [   4-1: 0] set_trig_src     ; //Alexei: changed
reg   [   3-1: 0] set_trig_src     ;
wire              ext_trig_p       ;
wire              ext_trig_n       ;
wire              asg_trig_p       ;
wire              asg_trig_n       ;
...
if (adc_rstn_i == 1'b0) begin
   ...
   adc_rst_do    <= 1'b0 ;
   adc_trig_sw   <= 1'b0 ;
//   set_trig_src  <= 4'h0 ;//Alexei: changed
   set_trig_src  <= 3'h0 ;
   adc_trig      <= 1'b0 ;
end else begin
...
//         set_trig_src <= sys_wdata[3:0] ;//Alexei: changed
         set_trig_src <= sys_wdata[2:0] ;
...
//       4'd8 : adc_trig <= asg_trig_p    ; // ASG - rising edge //Alexei: removed
//       4'd9 : adc_trig <= asg_trig_n    ; // ASG - falling edge //Alexei: removed
...
/*Alexei: disabled (controlled from Lock via trig_ext)
   //----------- ASG trigger
   // synchronize FFs
...
//assign asg_trig_p = (asg_trig_dp == 2'b01) ; // Alexei removed
//assign asg_trig_n = (asg_trig_dn == 2'b10) ; // Alexei removed
...
//     20'h00004 : begin sys_ack <= sys_en;          sys_rdata <= {{32- 4{1'b0}}, set_trig_src}       ; end //Alexei changed
     20'h00004 : begin sys_ack <= sys_en;          sys_rdata <= {{32- 3{1'b0}}, set_trig_src}       ; end 
...
//   Ning: extended & combined channel buffers
//         left 2 bytes correspond to channel A, right 2 bytes corrrespond to channel B
     20'h1????,
     20'h2????,
     20'h3????,
     20'h4????,
     ...
     20'h6????,
     20'h7????,
     20'h8???? : begin sys_ack <= adc_rd_dv;       sys_rdata <= {2'h0, adc_a_rd,   2'h0, adc_b_rd}  ; end
