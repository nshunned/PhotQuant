...
module increment_lin_unsign #(
   parameter     poswidth  = 14
    )
    (
    input clk,
    input rst,
        ...
        begin
            ...
            begin
                if (selmode & (~step[10]))// increase increment rate
                    ...
                    if ( &(~sum[14:poswidth])) // positive-unsaturated sum: increase counter
                        count <= sum[13:0];
            ...
            begin
                if (selmode & (~step[0]))// decrease increment rate
                    ...
                    if (~diff[14]) // positive difference : decrease counter
                        count <= diff[13:0];
                    else
                        count <= 14'b0; //saturates at 0
            end
        end    
    end    
end
