...
module LPFilterGain1 
  #(parameter Rin=14,//bit width of input
   ...
   (input clk, rst, run,
    ...
    output reg  signed  [Rin+MaxTau+1 -1:0]     dout,//derivative of output low-passed signal, with Rin-Sfix+1 integer and Sfix+MaxTau fractional bits
    output      signed  [Rin+Sfix+MaxTau -1:0]  out//output low-passed signal, with Rin integer and Sfix+MaxTau fractional bits
    );

reg  signed [Rin+Sfix+MaxTau+1 -1:0]    mem  ; // low-passed signal memory
wire signed [Rin+MaxTau -1:0]           in_div  ; // {in,fxp(Sfix+MaxTau)}/2^T,
wire signed [Rin+MaxTau -1:0]           out_div  ; // out/2^T,
