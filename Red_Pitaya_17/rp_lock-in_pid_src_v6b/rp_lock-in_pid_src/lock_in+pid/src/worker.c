...
void *rp_osc_worker_thread(void *args)
{
    ...
    while(1) {
        ...
        if(!long_acq || long_acq_idx == 0) {
            ...
            rp_osc_meas_period(&ch1_meas, &ch2_meas, &rp_fpga_cha_signal[0], &rp_fpga_chb_signal[0], dec_factor);
                    ...
                    float ch1_user_dc_off, float ch2_user_dc_off)
{
    ...
    for(out_idx=0; out_idx < OSC_FPGA_SIG_LEN; out_idx++) {
        // Ning: extract individual channel data from each 32-bit sample
        rp_osc_meas_min_max(ch1_meas,  in_cha_signal[out_idx] & 0x0000FFFF     );
        rp_osc_meas_min_max(ch2_meas, (in_chb_signal[out_idx] & 0xFFFF0000)>>16);
    ...
    for(out_idx=0, t_idx=0; out_idx < SIGNAL_LENGTH;
        ...
        if( rp_osc_params[LOCK_OSC_RAW_MODE].value>0 ){
            // Ning: extract individual channel data from each 32-bit sample
            cha_s[out_idx] = osc_fpga_cnv_cnt_to_v( in_cha_signal[in_idx] & 0x0000FFFF      , (float)(1<<(c_osc_fpga_adc_bits-1)) , 0, 0);
            chb_s[out_idx] = osc_fpga_cnv_cnt_to_v((in_chb_signal[in_idx] & 0xFFFF0000)>>16 , (float)(1<<(c_osc_fpga_adc_bits-1)) , 0, 0);
            ...
            // Ning: extract individual channel data from each 32-bit sample
            cha_tmp = osc_fpga_cnv_cnt_to_v( in_cha_signal[in_idx] & 0x0000FFFF      , ch1_max_adc_v, fe_ch1_dc_offs, ch1_user_dc_off);
            chb_tmp = osc_fpga_cnv_cnt_to_v((in_chb_signal[in_idx] & 0xFFFF0000)>>16 , ch2_max_adc_v, fe_ch2_dc_offs, ch2_user_dc_off);

            // cha_tmp = osc_fpga_cnv_cnt_to_v(in_cha_signal[in_idx], ch1_max_adc_v, fe_ch1_dc_offs, ch1_user_dc_off);
            // chb_tmp = osc_fpga_cnv_cnt_to_v(in_chb_signal[in_idx], ch2_max_adc_v, fe_ch2_dc_offs, ch2_user_dc_off);
                            ...
                            float ch1_user_dc_off, float ch2_user_dc_off)
{
    ...
    for(; in_idx < curr_ptr; in_idx++) {
        ...
        // Ning: extract individual channel data from each 32-bit sample
        rp_osc_meas_min_max(ch1_meas,  cha_in_signal[in_idx] & 0x0000FFFF     );
        rp_osc_meas_min_max(ch2_meas, (chb_in_signal[in_idx] & 0xFFFF0000)>>16);
    ...
    for(; (next_out_idx < SIGNAL_LENGTH); next_out_idx++,
        ...
        if( rp_osc_params[LOCK_OSC_RAW_MODE].value>0 ){
            // Ning: extract individual channel data from each 32-bit sample
            cha_out[next_out_idx] = osc_fpga_cnv_cnt_to_v( cha_in_signal[in_idx] & 0x0000FFFF      , (float)(1<<(c_osc_fpga_adc_bits-1)) , 0, 0);
            chb_out[next_out_idx] = osc_fpga_cnv_cnt_to_v((chb_in_signal[in_idx] & 0xFFFF0000)>>16 , (float)(1<<(c_osc_fpga_adc_bits-1)) , 0, 0);
            ...
            // Ning: extract individual channel data from each 32-bit sample
            cha_tmp = osc_fpga_cnv_cnt_to_v( cha_in_signal[in_idx] & 0x0000FFFF      , ch1_max_adc_v, fe_ch1_dc_offs, ch1_user_dc_off);
            chb_tmp = osc_fpga_cnv_cnt_to_v((chb_in_signal[in_idx] & 0xFFFF0000)>>16 , ch2_max_adc_v, fe_ch2_dc_offs, ch2_user_dc_off);

            // cha_tmp = osc_fpga_cnv_cnt_to_v(cha_in_signal[in_idx], ch1_max_adc_v, fe_ch1_dc_offs, ch1_user_dc_off);
            // chb_tmp = osc_fpga_cnv_cnt_to_v(chb_in_signal[in_idx], ch2_max_adc_v, fe_ch2_dc_offs, ch2_user_dc_off);
        ...
        /* DISABLED
        cha_tmp = osc_fpga_cnv_cnt_to_v(cha_in_signal[in_idx], ch1_max_adc_v, rp_calib_params->fe_ch1_dc_offs, ch1_user_dc_off);

        chb_tmp = osc_fpga_cnv_cnt_to_v(chb_in_signal[in_idx], ch2_max_adc_v, rp_calib_params->fe_ch1_dc_offs, ch2_user_dc_off);
                    ...
                    int ch1_probe_att, int ch2_probe_att, int ch1_gain, int ch2_gain, int en_avg_at_dec)
{
    ...
    for (iter=0; iter < 10; iter++) {
        ...
        for(smpl_cnt = 0; smpl_cnt < OSC_FPGA_SIG_LEN; smpl_cnt++) {
            // Ning: extract individual channel data from each 32-bit sample
            int cha_smpl =  rp_fpga_cha_signal[smpl_cnt] & 0x0000FFFF;
            int chb_smpl = (rp_fpga_chb_signal[smpl_cnt] & 0xFFFF0000)>>16;
    ...
    if(dy[channel] < c_noise_thr) {
       ...
       int wr_ptr_curr, wr_ptr_trig;
        ...
        for(time_range = 0; time_range < 5; time_range++) {
            ...
            // Ning: added parameter "int channel" for identification
            meas_period(&meas, channel, sig_data, wr_ptr_trig, dec_factor, &loc_min, &loc_max);
...
int rp_osc_meas_avg_amp(rp_osc_meas_res_t *ch_meas, int avg_len)
{
    ch_meas->avg /= avg_len;
    ch_meas->std /= avg_len;
    ch_meas->std  = ch_meas->std - pow(ch_meas->avg,2);
                       ...
                       int *in_cha_signal, int *in_chb_signal, int dec_factor)
{
    ...
    // Ning: added parameter "int channel" for identification
    meas_period(ch1_meas, 0, in_cha_signal, wr_ptr_trig, dec_factor, &min, &max);
    meas_period(ch2_meas, 1, in_chb_signal, wr_ptr_trig, dec_factor, &min, &max);
...
/*----------------------------------------------------------------------------------*/
// Ning: added parameter "int channel" for identification
int meas_period(rp_osc_meas_res_t *meas, int channel, int *in_signal, int wr_ptr_trig, int dec_factor,
                int *min, int *max)
...
//                 int *min, int *max)
{
    ...
    *min = INT_MAX;
    ...
    for(ix = 0; ix < (OSC_FPGA_SIG_LEN); ix++) {
        ...
        // Ning: added parameter "int channel" for identification
        int sa = rp_osc_adc_sign( (channel == 0) ? (in_signal[ix_corr] & 0x0000FFFF) : ((in_signal[ix_corr] & 0xFFFF0000)>>16) );
...
int rp_osc_meas_convert(rp_osc_meas_res_t *ch_meas, float adc_max_v, int32_t cal_dc_offs)
{
    ...
    ch_meas->std = rp_osc_meas_cnv_cnt(ch_meas->std, adc_max_v);
