...
/*
    int32_t m;
    if(cnts & (1 << (field_len - 1))) {    // check sign
        m = -1 *((cnts ^ ((1 << field_len) - 1)) + 1);        // negative number
    } else {        // positive number
        m = cnts;
    }
*/ //Alexei: disabled, sign handled by intExpand in seqpid.c

	int32_t m=cnts;
	/* check limits (Alexei: checking before offset subtraction instead of after, to deal with 0/+2V devices*/
...
float cmn_CnvCalibCntToV(uint32_t field_len, int32_t calib_cnts, float adc_max_v, bool calibFS_LO, float calibScale, float user_dc_off)
{
    /* map ADC counts into user units */
    double ret_val = ((double)calib_cnts * adc_max_v / (double)(1 << (field_len - 1)));
//    double ret_val = ((double)calib_cnts / (double)(1 << (field_len - 1)));

    /* and adopt the calculation with user specified DC offset */
    ret_val += user_dc_off;
    /* adopt the calculation with calibration scaling */ //Alexei: added conditioning on calibFS_LO for consistency with reciprocal voltage-to-count converter
//    ret_val *= (double)calibScale / ((double)FULL_SCALE_NORM/(double)adc_max_v);
    ret_val *= (double)calibScale / ((double)((!calibFS_LO) ? 1.f : (FULL_SCALE_NORM/adc_max_v)));
//    ret_val *= FULL_SCALE_NORM/((double)calibScale);
    return ret_val;
}
...
float cmn_CnvCntToV(uint32_t field_len, uint32_t cnts, float adc_max_v, bool calibFS_LO, uint32_t calibScale, int calib_dc_off, float user_dc_off)
{
    int32_t calib_cnts = cmn_CalibCnts(field_len, cnts, calib_dc_off);
    return cmn_CnvCalibCntToV(field_len, calib_cnts, adc_max_v, calibFS_LO, cmn_CalibFullScaleToVoltage(calibScale), user_dc_off);
}

float rp_cmn_CnvCntToV(uint32_t field_len, uint32_t cnts, float adc_max_v, bool calibFS_LO,  uint32_t calibScale, int calib_dc_off, float user_dc_off) {
...
//        voltage /= (float) cmn_CalibFullScaleToVoltage(calib_scale) / (float)(FULL_SCALE_NORM/adc_max_v);
	...
	//Alexei : removed this to deal with 0/+2V devices
	/*
